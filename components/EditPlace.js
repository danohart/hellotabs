...
export default function EditPlaceModal({
  ...
  return (
    <div className='fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4'>
      <div className='bg-white dark:bg-slate-800 rounded-lg shadow-lg w-full max-w-2xl max-h-screen overflow-y-auto'>
        ...
        <form onSubmit={handleSubmit} className='p-4'>
          ...
          {editData.events.map((event, eventIndex) => (
            ...
            >
              <h4 className='font-semibold mb-4'>Event {eventIndex + 1}</h4>

              {event.eventSchedule.map((schedule, scheduleIndex) => (
                <div
                  key={scheduleIndex}
                  className='mb-4 p-3 bg-gray-50 dark:bg-slate-700 rounded'
                >
                      ...
                      {daysOfTheWeek.map((day) => (
                        <label key={day} className='flex items-center text-sm'>
                          <input
                            type='checkbox'
                            className='mr-2'
                            checked={schedule.byDay.includes(DayOfWeek[day])}
                            onChange={() =>
