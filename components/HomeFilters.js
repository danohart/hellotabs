  ...
  return (
    <div className='flex gap-2 overflow-x-auto py-1 pb-3 scrollbar-purple'>
      <button
        ...
        className={`mb-4 shrink-0 flex items-center gap-1.5 px-4 py-2 rounded-full border font-semibold text-sm transition-colors ${
          activeNowOnly
      ...
      {POPULAR_NEIGHBORHOODS.map((neighborhood) => (
        <Link
          ...
          className='mb-4 shrink-0 px-4 py-2 rounded-full border border-gray-300 dark:border-slate-500 text-sm font-semibold text-gray-700 dark:text-gray-300 hover:border-purple-400 dark:hover:border-purple-500'
        >
          {neighborhood.name}
        </Link>
      ))}
